...
# Valid preference values for NVTE_PREFER
...
class PolicyManager:
    ...
    def _policy_from_env(self) -> SelectionPolicy:
        # Priority: NVTE_PREFER (highest) > NVTE_PREFER_VENDOR (legacy)
        #
        # NVTE_PREFER: Explicit preference by name (flagos, vendor, reference)
        # NVTE_PREFER_VENDOR: Legacy boolean flag (1=vendor, 0=flagos)

        prefer_str = None

        # 1. Check NVTE_PREFER first (highest priority)
        nvte_prefer = os.environ.get("NVTE_PREFER", "").strip().lower()
        if nvte_prefer:
            if nvte_prefer in VALID_PREFER_VALUES:
                prefer_str = nvte_prefer
            else:
                print(
                    f"[WARNING] Invalid NVTE_PREFER value: '{nvte_prefer}'. "
                    f"Valid values: {', '.join(sorted(VALID_PREFER_VALUES))}"
                )

        # 2. Fall back to NVTE_PREFER_VENDOR (legacy)
        if prefer_str is None:
            prefer_vendor = os.environ.get("NVTE_PREFER_VENDOR", "").strip()
            if prefer_vendor == "1":
        ...
        strict = os.environ.get("NVTE_STRICT", "0").strip() == "1"

        deny_str = os.environ.get("NVTE_DENY_VENDORS", "").strip()
        deny_vendors = self._parse_csv_set(deny_str) if deny_str else None

        allow_str = os.environ.get("NVTE_ALLOW_VENDORS", "").strip()
        allow_vendors = self._parse_csv_set(allow_str) if allow_str else None

        per_op_str = os.environ.get("NVTE_PER_OP", "").strip()
