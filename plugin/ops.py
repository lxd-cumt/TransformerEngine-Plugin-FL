...
class TEModule:
    ...
    def __repr__(self) -> str:
        ...
        return f"TEModule(operators={op_count}, manager={self._manager.__class__.__name__})"


# Global singleton instance
_global_te_module: Optional[TEModule] = None
_te_module_lock = None


def get_te_module() -> TEModule:
    """
    Get or create the global TEModule instance.

    This function returns a singleton TEModule that uses the default OpManager.
    The instance is created lazily on first access.

    Returns:
        The global TEModule instance

    Example:
        >>> import plugin
        >>> # Or explicitly:
        >>> from core.base import get_te_module
        >>> tex = get_te_module()
        >>> result = tex.rmsnorm_fwd(input, weight, eps=1e-5)
    """
    global _global_te_module, _te_module_lock

    if _global_te_module is None:
        # Import here to avoid issues at module load time
        import threading

        if _te_module_lock is None:
            _te_module_lock = threading.RLock()

        with _te_module_lock:
            if _global_te_module is None:
                _global_te_module = TEModule()

    return _global_te_module


def reset_te_module() -> None:
    """
    Reset the global TEModule instance.

    This is primarily useful for testing. After calling this function,
    the next call to get_te_module() will create a fresh instance.
...
def reset_registry() -> None:
    ...
    # Also reset the TEModule singleton since it depends on OpManager
    reset_te_module()
