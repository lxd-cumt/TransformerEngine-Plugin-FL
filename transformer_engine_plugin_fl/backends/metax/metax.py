...
def _check_metax_available() -> bool:
    ...
    try:
        from ._build_config import SKIP_METAX_BUILD  # noqa: will always fail

        if SKIP_METAX_BUILD:
            print("[Metax] Disabled: Metax was skipped at build time")
            return False
    except ImportError:
        if bool(int(os.environ.get("NVTE_SKIP_METAX", "0"))):
            print("[Metax] Disabled: NVTE_SKIP_METAX=1")
            return False
...
class MetaxBackend(BackendBase):
    @staticmethod
